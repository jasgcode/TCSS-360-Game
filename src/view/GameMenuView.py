...
class InGameMenuView:
    def __init__(self, screen, game_view):
        """
        Initialize the InGameMenuView with the screen and game view.
        """
        self.screen = screen
    ...
    def draw_menu(self, menu_options):
        """
        Draw the in-game menu options on the screen.
        """
    ...
    def draw_help_popup(self):
        """
        Draw the help popup on the screen.
        """
        popup_width = 700
        popup_height = 600
    ...
    def get_clicked_option(self, pos):
        ...
        """
        for i, option in enumerate(["Save", "Help", "Back", "Main Menu", "Exit Game"]):
            text = self.font.render(option, True, (0, 0, 0))
            text_rect = text.get_rect(center=(self.screen.get_width() // 2, 200 + i * 50))
